...
const char* host = "DormDaniSonoff2"; // nombre del entorno
...
void loop() {
  ...
  if (estado_pulsador==LOW)
  {
    ...
    {
      estado_luz=1;
      client.publish("DormDaniSonoff2","SwitchedOn");
      Serial.println("Switch On");
    ...
    {
      estado_luz=0; 
      client.publish("DormDaniSonoff2","SwitchedOff");
      Serial.println("Switch Off");
  ...
  if (conectado==1)
  {
    if (WiFi.status() == WL_CONNECTED) {
    if (!client.connected()) {
      if (client.connect("ESP8266: DormDaniSonoff2")) {
